...
(function (global, factory) {
	...
	(global.chimeeKernelMp4 = factory(global.fs,global.path));
...
var Mp4decode$1 = function (_CustEvent) {
		...
		value: function bindEvent() {
			...
			this.mp4box.onReady = function (info) {
				...
				info.tracks.forEach(function (item) {
					_this2.mp4box.setSegmentOptions(item.id, item.type);
				});

				var initSegs = _this2.mp4box.initializeSegmentation();
			...
			this.mp4box.onSegment = function (id, user, buffer, sampleNum) {
				_this2.onMediaSegment({ type: user, buffer: buffer });
			};
			this.mp4box.onSamples = function (id) {
				console.log(2);
			};
			this.mp4box.onItem = function (id) {
				console.log(3);
			};
			this.mp4box.start();
		}
...
var TransmuxerWorker = new src$1("./transmuxer-worker.js", function (window, document) {
  var self = this;
  var CPU = null;
  var loader = null;
  ...
  this.addEventListener('message', function (e) {
    ...
    switch (e.data.cmd) {
      ...
      case 'loadSource':
        ...
        break;
      case 'pipe':
        var consumed = CPU.setflv(e.data.source);
