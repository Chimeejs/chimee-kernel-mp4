...
var _chimeeHelper = __webpack_require__(2);
...
var MSEController = function (_CustEvent) {
  ...
  _createClass(MSEController, [{
    ...
    value: function init(mediaInfo) {
      if (this.mediaSource) {
        _chimeeHelper.Log.Error(this.tag, 'MediaSource has been attached to an HTMLMediaElement!');
    ...
    value: function onSourceOpen() {
      _chimeeHelper.Log.verbose(this.tag, 'MediaSource onSourceOpen');
    ...
    value: function addSourceBuffer(type) {
      ...
      _chimeeHelper.Log.verbose(this.tag, 'add sourcebuffer ' + type);
      var sb = this.sourceBuffer[type];
      sb.addEventListener('error', this.e.onSourceBufferError);
      sb.addEventListener('abort', function () {
        return _chimeeHelper.Log.verbose(_this2.tag, 'sourceBuffer: abort');
      });
    ...
    value: function doCleanupSourceBuffer(type) {
      _chimeeHelper.Log.verbose(this.tag, 'docleanBuffer');
      var currentTime = this.video.currentTime;
      var sb = this.sourceBuffer[type];
      var buffered = sb.buffered;
      var doRemove = false;
    ...
    value: function appendBuffer(data, type) {
      try {
        ...
        if (e.code === 22) {
          // chrome 大概会有350M
          _chimeeHelper.Log.verbose(this.tag, 'MediaSource bufferFull');
    ...
    value: function onSourceEnded() {
      _chimeeHelper.Log.verbose(this.tag, 'MediaSource onSourceEnded');
    }
    ...
    value: function onSourceClose() {
      _chimeeHelper.Log.verbose(this.tag, 'MediaSource onSourceClose');
    ...
    */
    ...
    value: function onSourceBufferError(e) {
      console.log(e);
      // Log.info(e);
      _chimeeHelper.Log.error(this.tag, 'SourceBuffer Error: ' + e);
    }
      ...
      if (this.mediaSource) {
        ...
        if (ms.readyState === 'open') {
          try {
            ...
            _chimeeHelper.Log.e(this.tag, error.message);
          }
        }
...
var _mp4decode = __webpack_require__(5);
  ...
  _createClass(Transmuxer, [{
    ...
    */
    ...
    value: function onDemuxError(type, info) {
      _chimeeHelper.Log.error(this.tag, 'DemuxError: type = ' + type + ', info = ' + info);
...
var _handleRange = __webpack_require__(1);
